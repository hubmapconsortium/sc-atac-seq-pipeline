...
def main(assay: Assay, fastq_dirs: Iterable[Path], orig_fastq_dir: Path, output_filename_prefix, output_dir: Path):
        ...
        if assay == Assay.MULTIOME_10X:
            ...
            if metadata_file is not None and metadata_file.is_file():
                with open(metadata_file, newline="") as f:
                    ...
                    offset=0
                    length=16
                    if("cell_barcode_offset" in metadata and metadata["cell_barcode_offset"].isdigit()):
        ...
        for fastq1_file, fastq2_file, barcode_file in all_fastqs:
            ...
            for i, (f1r, f2r, bar) in enumerate(
                ...
                if assay == Assay.SNARESEQ:
                    ...
                    #print("after correction multiome barcode is ", barcode_pieces)
...
if __name__ == "__main__":
    ...
    main(args.assay, args.fastq_dirs, args.orig_fastq_dir, args.output_filename_prefix, args.output_dir)
