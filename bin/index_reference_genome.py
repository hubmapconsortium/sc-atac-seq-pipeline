...
def index_reference(reference_genome: Path):
    ...
    samtools_command = [
        piece.format(reference_genome=reference_genome) for piece in SAMTOOLS_FAIDX_COMMAND
    ]
    print("Running", " ".join(snaptools_command))
...
def extract_alignment_index(alignment_index: Path) -> Path:
    ...
    index_dest_dir.mkdir()
    command = [
        piece.format(alignment_index=alignment_index) for piece in EXTRACT_ALIGNMENT_INDEX_COMMAND
    ]
