...
def main(directory: Path):
    ...
    #]
    #print("command is:{}".format(command))i
    ...
    for r1_fastq_file, r2_fastq_file, r3_fastq_file in find_fastq_files(directory):
        ...
        json_template_with_substitutes_str = SEQUENCES_TEMPLATE.safe_substitute(json_template_dict)
        #print("json template with substitutions is:{}".format(json_template_with_substitutes_str))

        json_template_with_substitutes = SEQUENCES_TEMPLATE.substitute(json_template_dict)
        #print("json template with substitutions using substitute is:{}".format(json_template_with_substitutes_str))
      
        # Convert string representation of dictionary to an python dictionary
        # https://stackoverflow.com/questions/988228/convert-a-string-representation-of-a-dictionary-to-a-dictionary
        #sequence_bundle = ast.literal_eval(json_template_with_substitutes)
        sequence_bundle = json.loads(json_template_with_substitutes)

        #pp = pprint.PrettyPrinter(indent=4)
        #pp.pprint(sequence_bundle)
        #sequence_bundle_list = []
        #sequence_bundle_list.append(sequence_bundle)
        #print("sequence bundle is:", json.dumps(sequence_bundle))
        sequence_file_bundles.append(sequence_bundle)
    #pp = pprint.PrettyPrinter(indent=4)
    #pp.pprint(sequence_file_bundles)
    #print("sequence_file_bundles is:{}".format(sequence_file_bundles))
